...
class BigQueryClient:
    """Minimal BigQuery client wrapper used by the Streamlit app.

    Public methods expected by `app.py`:
      - get_gke_costs_by_service(start_date: str, end_date: str) -> pd.DataFrame
      - test_table_connection() -> Tuple[bool, str]

    Use `use_live=True` to attempt real BigQuery queries. When False the client
    returns deterministic synthetic data and connectivity checks succeed.
    """
    ...
    def __init__(self, project: Optional[str] = None, use_live: bool = False, table: Optional[str] = None):
        ...
        if self.use_live:
            try:
                ...
                # Raise a clear error so callers (and UI) can present helpful
                # diagnostics rather than failing with obscure import errors.
    ...
    def get_gke_costs_by_service(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        - In mock mode (use_live=False) returns a small deterministic DataFrame.
        - In live mode runs an aggregation against the configured table.
        """
    ...
    def test_table_connection(self) -> Tuple[bool, str]:
        ...
        Returns (success, message). In mock mode this returns success:
        in live mode it executes a simple COUNT(1) to verify access.
        """
        if not self.use_live:
        ...
        try:
            ...
            # Return False and the raw message so the UI can display helpful
            # diagnostic text (e.g., permission denied, table not found, etc.).
            return False, str(e)
import os
from typing import Tuple, Optional
import pandas as pd
